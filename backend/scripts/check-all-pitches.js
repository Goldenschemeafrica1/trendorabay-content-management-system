...
async function checkAllPitches() {
  try {
    const [rows] = await db.query(
      'SELECT id, full_name, pitch_title, article_attachment, created_at FROM pitch_submissions ORDER BY created_at DESC LIMIT 10'
    );
    
    console.log('Recent pitch submissions:');
    console.log('========================');
